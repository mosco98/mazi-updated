...
const Navbar = ({ sideDrawerHandler }) => {
  ...
  return (
    <nav className="absolute w-full z-10 px-4 py-2 top-0 bg-white">
      <div className="flex items-center justify-between">
        <Menu size="35" className="cursor-pointer menu-btn" onClick={sideDrawerHandler} />
        <Link to="/">
        ...
        <div class="flex items-center nav-links">
          ...
          <Link to="/" className="text-white bg-gray-800 px-2 py-1 rounded font-medium mx-6">
            Pre-order
          </Link>
        </div>
        <div className="flex items-center">
          <Link to="/cart" className="text-gray-900 relative">
            <FiShoppingBag className="text-gray-900" size="35" strokeWidth="1.3" />
            {cartItems.length > 0 && (
              <span
          ...
          <FiUser className="text-gray-900 ml-6 user-icon" size="35" strokeWidth="1.3" />
        </div>
      </div>
    </nav>
  )
